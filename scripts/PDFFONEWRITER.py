...
A cover image named "cover.jpg" (if present in the input folder) is added as the first page.
...
# COVER PAGE LOGIC
# -------------------------------------------------------------------------------
def add_cover_page(pdf, input_folder, page_width, page_height):
    """
    Checks for a file named 'cover.jpg' in the input folder.
    If found, adds it as the first page of the PDF.
    """
    cover_path = os.path.join(input_folder, "cover.jpg")
    if os.path.exists(cover_path) and os.path.isfile(cover_path):
        logging.info("Adding cover page: %s", cover_path)
        pdf.drawImage(cover_path, 0, 0, width=page_width, height=page_height, preserveAspectRatio=True)
        pdf.showPage()
    else:
        logging.info("Cover page not found at %s", cover_path)
...
def main():
    ...
    # --- ADD COVER PAGE (if cover.jpg exists) ---
    add_cover_page(c, input_folder, page_width, page_height)
