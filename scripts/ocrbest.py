...
def process_images_to_json(directory, output_dir):
    ...
    all_pages = []
    chunk_number = 1  # Start chunk count
    ...
    for index, img_path in enumerate(image_files, start=1):
        ...
        # Define subsections with correct title handling
        subsections = []
        if text:
            title = ""  # Title is empty when only one section exists
            subsections.append({"title": title, "content": text})
        ...
        if index % 10 == 0 or index == len(image_files):  # Save at last page too
