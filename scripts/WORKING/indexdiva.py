...
import re
import argparse
import hashlib  # Added for compact id generation
...
STOP_WORDS = {
    ...
    "rappresenta", "tipo", "nome",

    # English stop words
    "the", "The", "and", "or", "but", "if", "then", "with", "a", "an", "of", "for", "some", "many", "much",
    "both", "follow", "in", "on", "at", "to", "by", "is", "are", "was", "were", "be", "been", "am", "eg",
...
def compact_id(original_id, length=8):
    """
    Generate a compact representation of the given identifier.
    Uses an MD5 hash and returns the first `length` hexadecimal digits.
    """
    digest = hashlib.md5(original_id.encode('utf-8')).hexdigest()
...
def extract_folder_info(folder_name):
    """
    Extract the main part and a unique identifier from a folder name.
    Tries first a dash-separated pattern; if that fails, falls back to space separation.
    Returns (main_part, folder_id) if a unique id is detected, otherwise (folder_name, None).
    """
...
def clean_directory_name(directory):
    """
    Return directory name if it contains only allowed characters (alphanumeric, dash, underscore, space).
    """
    if re.match(r'^[a-zA-Z0-9 _-]+$', directory):
        return directory
    return None

def clean_title_from_folder(folder_name):
    """
    Clean the folder name to derive a human-readable title and extract a unique identifier if present.
    For example, if the folder name is:
       "iaia-vs-cam-2cM9TdzTmU8dyw2pqoRLuntJd0c" or "iaia vs cam 2cM9TdzTmU8dyw2pqoRLuntJd0c",
    then this function returns:
       ("iaia vs cam", "2cM9TdzTmU8dyw2pqoRLuntJd0c")
    """
    main_part, folder_id = extract_folder_info(folder_name)
    title = main_part.replace("-", " ")
...
def index_files(root_dir):
    ...
    for dirpath, _, filenames in os.walk(root_dir):
        ...
        # For "chats" and "snippets", expect a single folder level.
        if parts and parts[0].lower() in ["chats", "snippets"]:
            type_field = parts[0].replace('-', ' ').title()
            entry_folder = None  # No second-level folder
        else:
            # Otherwise, expect a two-folder-level structure:
            if len(parts) >= 2:
                type_field = parts[0].replace('-', ' ').title()
        ...
        for filename in filenames:
            ...
            if filename.lower() == "novel.md":
                ...
                if entry_folder and cleaned_entry_folder:
                    ...
                    # If we have a folder id, convert it to a compact form:
                    if nested_code:
                        compacted = compact_id(nested_code)
                        entry_id = compacted
                        # Update nestedEntries with the compact id for consistency:
                        if "nestedEntries" in entry_metadata:
                            if isinstance(entry_metadata["nestedEntries"], list):
                                if compacted not in entry_metadata["nestedEntries"]:
                                    entry_metadata["nestedEntries"].append(compacted)
                            else:
                                entry_metadata["nestedEntries"] = [entry_metadata["nestedEntries"], compacted]
                        else:
                            entry_metadata["nestedEntries"] = [compacted]
                    else:
                        entry_id = None
                else:
...
def resolve_connections(entries):
    ...
    resolve them into a "connections" field containing slim connection objects.
    ...
    for entry in entries:
        ...
        for sub_id in entry.get("nestedEntries", []):
            # Skip if the sub_id matches the entry's own id.
            if sub_id == own_id:
                continue
            if sub_id in lookup:
        ...
        if entry.get("connections"):
            ...
            for conn in entry["connections"]:
                slim_connections.append({
                    "id": conn.get("id"),
                    "t": conn.get("title") if conn.get("title") else conn.get("t"),
                    "ty": conn.get("ty")
                })
