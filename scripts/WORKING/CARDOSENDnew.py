...
# ------------------------------------------------------------------------------
# Interactive prompt for INPUT_FOLDER, OUTPUT_FOLDER and PROMPT_FILE
# ------------------------------------------------------------------------------
default_input_folder = os.getenv("INPUT_FOLDER", "D:/EDIVA/cardonaproject/processed/1God/index")
default_output_folder = os.getenv("OUTPUT_FOLDER", "D:/EDIVA/cardonaproject/tradotto/1God/index")
default_prompt_file = os.getenv("PROMPT_FILE", "prompt.txt")

user_input = input(f"Inserisci la cartella di input [{default_input_folder}]: ").strip()
user_output = input(f"Inserisci la cartella di output [{default_output_folder}]: ").strip()
user_prompt = input(f"Inserisci il percorso del file del prompt [{default_prompt_file}]: ").strip()

# Se l'utente preme Invio, viene usato il valore di default
user_input_folder = user_input if user_input else default_input_folder
user_output_folder = user_output if user_output else default_output_folder
user_prompt_file = user_prompt if user_prompt else default_prompt_file

# Se l'utente ha modificato il valore, aggiorniamo le variabili d'ambiente nella sessione corrente
if user_input_folder != default_input_folder:
    os.environ["INPUT_FOLDER"] = user_input_folder
if user_output_folder != default_output_folder:
    os.environ["OUTPUT_FOLDER"] = user_output_folder
if user_prompt_file != default_prompt_file:
    os.environ["PROMPT_FILE"] = user_prompt_file

INPUT_FOLDER = user_input_folder
...
# ------------------------------------------------------------------------------
# Check that the input folder exists and create the output folder if needed
# ------------------------------------------------------------------------------
...
def process_file(file_path, relative_path):
    ...
    Process a file:
      ...
      - Otherwise, load the JSON, send it to GPT, clean the response, and verify that
    ...
    # Expect input JSON to be a list of pages.
    ...
    while attempt < max_attempts:
        ...
        try:
            ...
            last_attempt_output_text = output_text  # Save the latest output for potential fallback.
        except Exception as e:
    ...
    # If verification passed, use the normal output file name.
    # Otherwise, use a '.json.deb' suffix for comparison.
    if verified_output_text is None:
        ...
        try:
            ...
            logger.error(f"[red]Error writing fallback output file[/red] {fallback_file}: {e}")
        return "error"
