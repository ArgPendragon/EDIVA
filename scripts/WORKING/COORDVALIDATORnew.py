...
def prompt_arg(var_name, prompt_text):
    """
    Chiede all'utente di inserire un valore per una variabile, proponendo come default il valore
    presente nella variabile d’ambiente (se esistente). Se l’utente inserisce un nuovo valore, la
    variabile d’ambiente viene aggiornata.
    """
    default_value = os.environ.get(var_name, "")
    value = input(f"{prompt_text} [{default_value}]: ").strip() or default_value
    os.environ[var_name] = value
    return value

if __name__ == "__main__":
    # Utilizziamo le variabili ambientali per impostare i percorsi di default:
    # INPUT_FOLDER -> cartella contenente le immagini
    # OUTPUT_FOLDER -> percorso al file JSON
    json_path = prompt_arg("OUTPUT_FOLDER", "Inserisci il percorso al file JSON")
    images_dir = prompt_arg("INPUT_FOLDER", "Inserisci la cartella delle immagini")

    review_image_pages(json_path, images_dir)
