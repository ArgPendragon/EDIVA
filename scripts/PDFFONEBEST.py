...
def draw_adaptive_text(pdf, text, area, max_font_size=10, font_name="Times-Roman",
    ...
    Special formatting:
      ...
      - Then the main text is processed line by line (splitting on "\n"):
            * For each non‐empty line that (after stripping) exactly matches one of the header texts
              provided in the "headers" field (case‑insensitive), the line is rendered as a header:
                  - A blank line is inserted above,
                  - The line is drawn in bold (using font size increased by 2) and centered,
                  - And a blank line is inserted below.
            * All other lines are rendered normally, with wrapping (and justification if requested).

    The text color is reset to black at the start.
    """
    from reportlab.pdfbase.pdfmetrics import stringWidth  # in case not imported already
    pdf.setFillColorRGB(0, 0, 0)
    x, y, width, height = area

    # --- Spacing constants ---
    BLANK_LINE = max_font_size  # the height used for a blank line
    ...
    # --- Function to compute the required height ---
    def required_height(font_size):
        total = 0
        # Account for capitolo header block if present.
        if capitolo:
        ...
        for line in text.split("\n"):
            ...
            if sline == "":
                ...
                # For a header line, add a blank line above, then the header line, then a blank line below.
                header_font = font_size + HEADER_FONT_INCREMENT
                total += font_size      # blank line above
                total += header_font + line_spacing  # header line height
                total += font_size      # blank line below
            else:
                # For normal text, wrap the line.
...
# (Legacy optimized subtraction using shapely is kept here for reference.)
def subtract_rectangles_optimized(full_area, exclusions):
    """
    Given a starting rectangle (x, y, w, h) and a list of exclusion rectangles,
    returns a list of remaining rectangles.
    """
...
def layout_page(pdf, page, images_dir, page_width, page_height, last_text_page, temp_dir):
    ...
    if cap_coords:
        # If the first element is a number, assume a single caption.
    ...
    # Incorporate hints from image_configuration.
    img_conf = page.get("image_configuration", "").lower()
    if img_conf == "image on left":
        # If the image is on the left, choose candidate areas with larger x (to the right) first.
        candidate_areas = sorted(candidate_areas, key=lambda r: r[0], reverse=True)
    elif img_conf == "image on right":
        # If the image is on the right, choose candidate areas with smaller x (to the left) first.
        candidate_areas = sorted(candidate_areas, key=lambda r: r[0])
    else:
        # Default sorting: left to right.
        candidate_areas = sorted(candidate_areas, key=lambda r: r[0])
    logging.debug(f"Candidate areas after sorting by image_configuration: {candidate_areas}")

    if candidate_areas:
        # Optionally clear candidate areas.
    ...
    if len(chosen_areas) == 1:
        draw_adaptive_text(pdf, text_content, chosen_areas[0], max_font_size=10, alignment="justify",
